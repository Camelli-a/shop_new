...
| 功能模块 | 实现方式 |
|----------|----------|
| 后台登录 | 独立登录页面，管理员登录后保存 token 到 localStorage，路由守卫保护后台页面 |
| 权限管理 | 基于角色的权限控制，角色有超级管理员、管理员、普通用户，超级管理员不可删除，权限包含首页、商品、分类、订单、用户、角色管理 |
...
| 角色管理 | 角色列表、添加角色、编辑角色名称和描述、配置角色权限、删除角色（超级管理员角色不可删除） |
