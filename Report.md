...
| 周锦泽 | 23301087 | 前台首页与商品发现模块：首页、搜索框、轮播图、热门商品、顶部导航/底部导航的编码；补充首页商品分页/滚动加载；负责首页模块相关单元测试和文档撰写和PPT制作 | 20% |
| ___ | ___ | ___ | ___% |
...
后台管理端采用 React 组件化设计，基于路由保护、权限控制和数据管理分层实现，主要包含以下核心模块：
- 路由管理、权限控制、后台布局、各业务管理页面

### 4.2 功能模块详细说明

| 功能模块 | 实现方式 |
|----------|----------|
| 后台登录 | 独立登录页面（AdminLoginPage），管理员登录后保存 token 到 localStorage，路由守卫保护后台页面，登录接口返回用户权限列表和角色名称，提供三个测试账号：admin1（超级管理员）、admin2（商品管理员）、admin3（订单管理员），默认密码均为 123456 |
| 权限管理 | 基于角色的权限控制（RBAC），通过 AuthAdminContext 管理登录状态和权限，PermissionGuard 路由守卫，无权限跳转 403 页面，菜单动态显示用户有权限访问的菜单项，权限包含 dashboard（首页）、goods（商品）、categories（）、orders（订单）、users（用户）、roles（角色）管理，修改角色权限后自动刷新当前登录用户权限 |
| 仪表盘 | AdminHomePage 实现数据可视化，包含 6 个统计卡片：用户总数、今日新增用户、订单总数、今日订单、商品总数、在售商品，以及总收入统计、今日收入展示，最近订单列表展示，点击可查看订单详情模态框 |
| 商品管理 | ProductManagement 实现完整 CRUD 功能，支持分页、关键词搜索、分类筛选，添加/编辑商品时支持拖拽上传图片，使用 FileReader 读取图片为 base64，支持上架/下架商品，删除前二次确认，商品信息包括：商品名称、分类、价格、原价、库存、图片、描述、状态 |
| 分类管理 | CategoryManagement 实现分类增删改查，支持拖拽上传图片，支持 URL 输入或直接输入 URL，分类信息包括：分类名称、图标、排序、状态，删除分类时有商品的分类不可删除 |
| 订单管理 | OrderManagement 实现查看所有订单、按状态筛选（待付款/待发货/已发货/已完成/已取消），查看订单详情，支持更新订单状态（发货、完成等），订单详情包括：基本信息、用户信息、收货信息、商品信息、支付信息、物流信息 |
| 用户管理 | UserManagement 实现用户列表、添加用户、编辑用户信息、分配角色、启用/禁用用户、删除用户（超级管理员不可删除），用户信息包括：头像、用户名、姓名、邮箱、手机号、角色、状态 |
| 角色管理 | RoleManagement 实现角色列表、添加角色、编辑角色名称和描述、配置角色权限、删除角色（超级管理员角色不可删除），修改权限后自动刷新当前登录用户权限，权限配置为多选框形式配置，支持：首页、商品、分类、订单、用户、角色管理 |
| 403 页面 | ForbiddenPage 无权限访问时显示美观的 403 页面，提供返回首页和重新登录功能 |

### 4.3 核心技术实现

#### 4.3.1 权限控制实现
- 后端：登录时根据用户 role 字段匹配对应角色，返回用户权限列表 permissions 给前端
- 前端：AuthAdminContext 管理登录状态和权限，PermissionGuard 路由守卫，无权限显示 403 页面
- 权限映射：
  - dashboard：首页
  - goods：商品管理
  - categories：分类管理
...
#### 4.3.2 后台布局设计
- AdminLayout 实现侧边栏导航，包含 Logo、用户信息、菜单列表，菜单根据权限动态显示，支持折叠/展开，菜单高亮

#### 4.3.3 数据交互
- 使用 fetch API 与后端交互，统一的响应处理，错误处理，加载状态管理

#### 4.3.4 图片上传
- 使用 FileReader API 将图片转换为 base64 存储，支持拖拽上传和点击上传
